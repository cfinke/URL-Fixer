		...
		if (urlbar) {
			...
			urlbar.onTextEntered = function (justCorrect) {
				...
				if (goAhead) {
					...
					if (this.value != getShortcutOrURI(this.value, {})){
						goAhead = true;
					}
				}
				...
				if (goAhead){
					...
					if (urlValue != oldValue.toLowerCase()){
						...
						if (URLFIXER.prefs.getBoolPref("askFirst")){
							...
							var button = prompts.confirmEx(window, URLFIXER.strings.getString("urlfixer.confirmTitle"), URLFIXER.strings.getString("urlfixer.didYouMean").replace("#",urlValue), flags, URLFIXER.strings.getString("urlfixer.yes"), URLFIXER.strings.getString("urlfixer.cancel"), URLFIXER.strings.getString("urlfixer.no"), null, check);
			...
			if (goButton){
				...
				if (goButton.getAttribute("oncommand")) goButton.setAttribute("oncommand","if (document.getElementById('urlbar').onTextEntered(true)) { " + goButton.getAttribute("oncommand") + "}");
				if (goButton.getAttribute("onclick")) goButton.setAttribute("onclick","if (document.getElementById('urlbar').onTextEntered(true)) { " + goButton.getAttribute("onclick") + "}");
			}
		}
	},
